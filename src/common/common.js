document.addEventListener("DOMContentLoaded", function () {
  if (navigator.userAgent.match(/PINE/)) {
    if (/Android/i.test(navigator.userAgent)) {
      window.AosConnector.hideTitleBar();
    }
    if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
      webkit.messageHandlers.hideTitleBar.postMessage(""); //ios
    }
  }
});
  ...
  if (pineAppChk()) {
    ...
    if (isMobile()) {
      if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
        ...
        location.href =
          "https://play.google.com/store/apps/details?id=com.hamc.android.pine";
      }
    } else {
...
function appLinkFunc(str, num = "") {
  if (str == "web") {
    var pine_url =
      "hamcpine://share?&what=" + str + "&value=" + location.origin + "/" + num;
  } else {
