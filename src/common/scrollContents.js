...
window.addEventListener("DOMContentLoaded", function () {
  ...
  //SNS 버튼
  const titleBar = this.document.querySelector(".title-bar");
  ...
  document.addEventListener("scroll", function (event) {
    ...
    // var openLinkImgPos =
    //   linkCoverFull.offsetTop - currentScrollTop + (basicFoot.scrollHeight - 2);
    // var closeLinkImgPos = linkCoverFull.offsetTop - currentScrollTop;

    // // 하단 이미지 시작시 이미지 scale 변화
    // if (
    //   windowHeight - openLinkImgPos >= 0 &&
    //   windowHeight - openLinkImgPos <= windowHeight
    // ) {
    //   var linkImgScale =
    //     1 + (windowHeight - openLinkImgPos) / (windowHeight * 3);
    //   linkImg.style.transform = `scale(${linkImgScale})`;
    // }

    //프로그래스 바
    // progressBar.style.width =
    //   Math.ceil((currentScrollTop / (body.scrollHeight - windowHeight)) * 100) +
    //   "%";
    ...
    //     basicFoot.style.backgroundColor =
